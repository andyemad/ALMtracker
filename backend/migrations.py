...
What this script does (in order):
    ...
    Step 12 One-time cleanup: delete false 'removed' events from the partial
            scrape that ran on 2026-04-09 ~05:14 UTC (only 2,500 vehicles fetched
            instead of ~7,200, causing ~4,600 false removal events).
...
def _step12_cleanup_false_removals_april9(conn) -> None:
    """
    One-time cleanup: delete the ~4,600 false 'removed' events generated by a
    partial scrape on 2026-04-09 between ~05:00-05:30 UTC.

    That run fetched only 2,500 vehicles (pagination stopped at page 5 due to a
    transient HTTP error) instead of ~7,200, causing thousands of live vehicles
    to be falsely marked as removed.  Subsequent scrapes restored the correct
    active state, but the Activity Log remained polluted with false events.

    Guard: only deletes if ≥500 'removed' events exist in the narrow time window
    (confirms this is a mass false-removal, not normal churn).
    """
    count = conn.execute(text("""
        SELECT COUNT(*) FROM vehicle_events
        WHERE event_type = 'removed'
          AND timestamp >= '2026-04-09 05:00:00'
          AND timestamp <  '2026-04-09 05:30:00'
    """)).scalar()

    if count < 500:
        logger.info(
            f"Step 12: {count} removed events in window — no mass false-removal found, skipping"
        )
        return
    ...
    """))
    logger.info(
        f"Step 12: Deleted {result.rowcount} false 'removed' events "
        "from 2026-04-09 partial scrape"
    )
...
def run_migrations(engine: Engine) -> None:
    ...
    with engine.connect() as conn:
        ...
        _step12_cleanup_false_removals_april9(conn)
